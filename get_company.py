...
try:
    ...
    while ID <= 8:
        ...
        all_company = b.find_all('span', class_='hazardC_pro_con_company')
        ...
        for c in all_company:  # 获取产品对应的公司
            new_company.append(c.text.strip())
            count = count + 1
            print(count, names[i], '  ', new_company[i], '  ', new_urls)  # 输出产品名，公司，及url
            i = i + 1
        ID = ID + 1
except:
    print("网络出现错误或连接超时！")
